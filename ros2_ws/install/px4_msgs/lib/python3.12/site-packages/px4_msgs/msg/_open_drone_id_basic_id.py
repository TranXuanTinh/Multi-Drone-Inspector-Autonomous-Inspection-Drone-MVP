    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'px4_msgs.msg.OpenDroneIdBasicId')
            logger.debug(
...
class OpenDroneIdBasicId(metaclass=Metaclass_OpenDroneIdBasicId):
    ...
    def __init__(self, **kwargs):
        ...
        if 'uas_id' not in kwargs:
            self.uas_id = numpy.zeros(20, dtype=numpy.uint8)
        else:
    ...
    def uas_id(self, value):
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                ...
                assert value.size == 20, \
                    "The 'uas_id' numpy.ndarray() must have a size of 20"
                self._uas_id = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, int) for v in value) and
                 all(val >= 0 and val < 256 for val in value)), \
                "The 'uas_id' field must be a set or sequence with length 20 and each value of type 'int' and each unsigned integer in [0, 255]"
