...
px4_msgs__msg__ActuatorServosTrim__create(void)
{
...
px4_msgs__msg__ActuatorServosTrim__Sequence__init(px4_msgs__msg__ActuatorServosTrim__Sequence * array, size_t size)
{
  ...
  if (size) {
    if (size > SIZE_MAX / sizeof(px4_msgs__msg__ActuatorServosTrim)) {
      return false;
    }
  ...
  px4_msgs__msg__ActuatorServosTrim__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    if (input->size > SIZE_MAX / sizeof(px4_msgs__msg__ActuatorServosTrim)) {
      return false;
    }
    ...
    px4_msgs__msg__ActuatorServosTrim * data =
      (px4_msgs__msg__ActuatorServosTrim *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__ActuatorServosTrim__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__ActuatorServosTrim__fini(&output->data[i]);
        }
        return false;
      }
    }
